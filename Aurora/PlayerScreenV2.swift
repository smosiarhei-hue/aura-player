...
struct PlayerScreenV2: View {
    ...
    private func setupVideoLooper(url: URL) {
        teardownVideoLooper(); let player = AVQueuePlayer(playerItem: AVPlayerItem(url: url)); player.isMuted = true; player.actionAtItemEnd = .none
        videoLooper = AVPlayerLooper(player: player, templateItem: AVPlayerItem(url: url)); videoLooperPlayer = player
