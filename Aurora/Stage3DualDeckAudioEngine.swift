...
final class DualDeckAudioEngine: @unchecked Sendable {
    ...
    private final class Slot: @unchecked Sendable {
        ...
        var ended = false
        var generation = UUID()
        var spilloverTask: Task<Void, Never>?
    ...
    init() throws {
        for s in [a, b] {
            graph.attach(s.player)
            graph.attach(s.timePitch)
            graph.attach(s.dryMixer)
            graph.attach(s.eq)
            graph.attach(s.delay)
            graph.attach(s.mixer)
            graph.connect(s.player, to: s.timePitch, format: nil)
            graph.connect(s.timePitch, to: s.dryMixer, format: nil)
            graph.connect(s.dryMixer, to: s.eq, format: nil)
            graph.connect(s.eq, to: s.delay, format: nil)
            graph.connect(s.delay, to: s.mixer, format: nil)
            graph.connect(s.mixer, to: graph.mainMixerNode, format: nil)
            neutral(s)
        }
        graph.attach(userEQ)
        configureUserEQ()
        ...
        graph.mainMixerNode.installTap(onBus: 0, bufferSize: 2048, format: nil, block: Self.handleSpectrumTap)
    }
    ...
    func play(_ deck: Deck) async throws {
        let s = slot(deck)
        guard s.prepared, s.file != nil else { throw AudioEngineCoreError.deckNotPrepared(deck) }
        if !graph.isRunning {
            graph.prepare()
            try graph.start()
        }
    ...
    func pause(_ deck: Deck) async {
        let s = slot(deck)
        s.lastPosition = currentPosition(s)
        s.player.pause()
        s.playing = false
    }
    ...
    func stop(_ deck: Deck) async {
        ...
        s.playing = false
        s.ended = false
        s.mixer.outputVolume = 0
        s.dryMixer.outputVolume = 1
        neutral(s)
    }
    ...
    func setRate(_ rate: Float, for deck: Deck) async {
        let v = min(1.30, max(0.70, rate.isFinite ? rate : 1))
        slot(deck).rate = v
        slot(deck).timePitch.rate = v
    }

    func setPitch(_ cents: Float, for deck: Deck) async {
        let s = slot(deck)
        let clamped = min(200, max(-200, cents.isFinite ? cents : 0))
        s.timePitch.pitch = clamped
    }

    func rampPitch(_ deck: Deck, from: Float, to: Float, duration: Double) async {
        guard duration > 0 else { await setPitch(to, for: deck); return }
        let t0 = CACurrentMediaTime()
        while true {
            let elapsed = CACurrentMediaTime() - t0
            let p = min(1, max(0, Float(elapsed / duration)))
            let current = from + (to - from) * (1 - pow(1 - p, 2))
            await setPitch(current, for: deck)
            if p >= 1 { break }
    ...
    func applyEffect(_ kind: FxKind, value: Float, param: Float?, bpm: Float, to deck: Deck) async {
        ...
        case .highPass:
            let x = s.eq.bands[1]
            x.frequency = min(18000, max(20, value))
            x.bypass = value <= 21
            // Динамический подъем резонанса Q (сужение bandwidth от 0.8 до 0.18 при разгоне вверх)
            if value > 500 {
                let norm = min(1.0, max(0.0, (value - 500) / 4000))
                x.bandwidth = Float(0.8 - norm * 0.62)
            } else {
                x.bandwidth = 0.8
            }
        case .lowPass:
            let x = s.eq.bands[2]
            x.frequency = min(20000, max(100, value))
            x.bypass = value >= 19900
        case .bassKill, .bassOn:
            s.eq.bands[0].gain = min(0, max(-40, -40 * min(1, max(0, value))))
        case .echoOut:
            s.delay.wetDryMix = min(75, max(0, value))
            s.delay.feedback = min(60, max(0, param ?? 30))
            s.delay.delayTime = bpm > 0 ? min(2, max(0.05, 60 / Double(bpm) * 0.75)) : 0.375
        case .rateRamp:
            await setRate(value, for: deck)
        case .tapeStop:
            let clampedRate = max(0.015, min(1.0, value))
            await setRate(clampedRate, for: deck)
        case .stutter:
            s.dryMixer.outputVolume = max(0, min(1, value))
        case .reverbWash:
            s.delay.wetDryMix = min(90, max(0, value))
            s.delay.feedback = min(80, max(0, param ?? 60))
            s.delay.delayTime = bpm > 0 ? min(2, max(0.05, 60 / Double(bpm) * 0.375)) : 0.25
        case .volume:
    ...
    func startDelaySpillover(_ deck: Deck, duration: Double = 2.8) {
        let s = slot(deck)
        s.spilloverTask?.cancel()
        guard s.delay.wetDryMix > 1 else {
            s.mixer.outputVolume = 0
            s.dryMixer.outputVolume = 1
            neutral(s)
            return
        ...
        s.dryMixer.outputVolume = 0
        s.delay.wetDryMix = 100
        s.delay.feedback = min(s.delay.feedback, 45)
        ...
        s.spilloverTask = Task { @MainActor [weak self, weak s] in
            ...
            while !Task.isCancelled {
                let elapsed = CACurrentMediaTime() - t0
                let u = min(1.0, max(0.0, elapsed / duration))
                guard s.generation == generation else { return }
                let factor = Float(exp(-3.5 * u) * (1.0 - u))
                s.mixer.outputVolume = max(0, startVol * factor)
                if u >= 1.0 { break }
                do { try await ContinuousClock().sleep(for: .milliseconds(15)) } catch { return }
            }
            guard s.generation == generation else { return }
            s.mixer.outputVolume = 0
            s.dryMixer.outputVolume = 1
    ...
    func crossfade(from outgoing: Deck, to incoming: Deck, durationSeconds: Double, alignedToCueSeconds cue: Double) async throws {
        ...
        inc.spilloverTask?.cancel()
        inc.spilloverTask = nil

        let positionA = currentPosition(out)
        let remaining = cue - positionA
        out.dryMixer.outputVolume = 1
        out.mixer.outputVolume = 1
        inc.dryMixer.outputVolume = 1
        inc.mixer.outputVolume = 0
        ...
        do {
            while true {
                ...
                out.dryMixer.outputVolume = Float(cos(p * .pi / 2))
            ...
            out.mixer.outputVolume = 1
            inc.mixer.outputVolume = 0
            inc.lastPosition = currentPosition(inc)
            inc.player.pause()
            inc.playing = false
            await resetEffects(outgoing)
            await resetEffects(incoming)
            throw error
        }

        out.dryMixer.outputVolume = 0
        out.player.stop()
        out.playing = false

        inc.mixer.outputVolume = 1
        inc.dryMixer.outputVolume = 1
        await resetEffects(incoming, preservingRate: true)

        startDelaySpillover(outgoing, duration: 2.8)
    }
    ...
    private func neutral(_ s: Slot) {
        s.dryMixer.outputVolume = 1
        let bass = s.eq.bands[0]
        ...
        s.timePitch.rate = 1
        s.timePitch.pitch = 0
        s.timePitch.overlap = 8
    }

    private func rendered(_ s: Slot) -> Double {
    ...
    private func currentPosition(_ s: Slot) -> Double {
        guard s.duration > 0 else { return 0 }
        if s.ended { return s.duration }
        return min(s.duration, max(s.start, s.start + max(0, rendered(s))))
    }
    ...
    private func snapshot(_ s: Slot) -> DeckPlaybackSnapshot {
        ...
        if s.ended {
            ...
            p = min(s.duration, max(s.start, s.lastPosition))
        }
        let effectiveGain = s.mixer.outputVolume * s.dryMixer.outputVolume
        return DeckPlaybackSnapshot(
            deck: s.deck,
            fileURL: s.url,
            isPrepared: s.prepared,
            isPlaying: s.playing,
            gain: effectiveGain,
            queuedChunks: s.prepared && !s.ended ? 1 : 0,
            reachedEndOfFile: s.ended,
            positionSeconds: p,
            durationSeconds: s.prepared ? s.duration : nil
        )
    }

    private func slot(_ d: Deck) -> Slot {
        d == .a ? a : b
    }
}
