...
final class PlayerCore {
    ...
    private func scheduleTransitionIfNeeded() {
        ...
        // Gated on `!isSelectingNextTrack`: the harmonic/tempo smart-pick
        // above may still be resolving which track is actually `nextTrack`.
        // Starting to plan (or pre-buffer, below) against the plain
        // next-in-queue pick before that resolves let a late `plannedNextTrack`
        // swap desync `activeTransitionPlan`/`prebufferedTrackId` from the
        // real target - the hand-off guards further down then rejected the
        // mismatch and silently cancelled the transition, which is what made
        // AutoMix just fade the current track and hard-cut to the next one
        // with no audible mashup.
        if remaining <= 65.0, activeTransitionPlan == nil, !isPlanningTransition, !isSelectingNextTrack {
        ...
        if nextTrack.isStream, remaining <= prebufferThreshold, prebufferedTrackId != nextTrack.id, !isPrebufferingNextStream, !isSelectingNextTrack {
    ...
    private func tickTransition() {
        ...
        let outReverbMix = AutoMixDJEngine.sampleEnvelope(actions, target: "