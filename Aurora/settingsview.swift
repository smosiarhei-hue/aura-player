        ...
        NavigationStack {
            Form {
                ...
                Section {
                    ...
                    Text("NeuroMix работает с локальными файлами и выполняет DJ-переходы на устройстве. Онлайн-треки сохраняют AVPlayer-путь для Spatial Audio; для них используется обычное воспроизведение.")
                }
            ...
            .background(AuraScreenBackground(colors: [AG.bgRaised, AG.bg, AG.card], showsMesh: false))
            .listRowBackground(AG.card.opacity(0.78))
            .listRowSeparatorTint(AG.ink.opacity(0.10))
