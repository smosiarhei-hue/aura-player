# Path: .github/scripts/send_video_telegram.py
"""Deliver a simulator video via Telegram without logging credentials or response bodies."""
import http.client
import json
import os
...
MESSAGES = {
    ...
    "missing_video": "The video file is missing or empty.",
    "video_too_large": "Video exceeds the 50 MB upload limit; download the GitHub Actions artifact.",
    ...
    "success": "Telegram confirmed the video document message.",
}
...
def configuration(env):
    ...
    caption = ("Sonivo — видео превью на iPhone-симуляторе.\n"
               f"Коммит: {sha[:7]}\n"
...
def deliver(env, path, transport=post_document):
    ...
    if not path.is_file() or path.stat().st_size == 0:
        raise DeliveryError("missing_video")
    if path.stat().st_size > MAX_FILE_BYTES:
        raise DeliveryError("video_too_large")
...
def record_status(code, env):
    ...
    if env.get("GITHUB_STEP_SUMMARY"):
        with open(env["GITHUB_STEP_SUMMARY"], "a", encoding="utf-8") as output:
            output.write(f"## Telegram video delivery\n\n**{code}** — {message}\n")


def main(env=None, path=None, transport=post_document):
    env = os.environ if env is None else env
    path = Path("Sonivo-sim.mp4") if path is None else path
    try:
