# Path: .github/scripts/test_send_video_telegram.py
import contextlib
...
import send_video_telegram as sender
...
class VideoDeliveryTests(unittest.TestCase):
    def setUp(self):
        ...
        self.path = Path(self.directory.name) / "Sonivo-sim.mp4"
        self.path.write_bytes(b"\x00\x00\x00\x18ftypmp42test-video\x00\xff")
    ...
    def test_success_uploads_exact_file_and_simulator_caption(self):
        ...
        self.assertEqual(parts["document"].get_filename(), "Sonivo-sim.mp4")
        caption = parts["caption"].get_payload(decode=True).decode()
        self.assertIn("симуляторе", caption)
    ...
    def test_missing_or_empty_file_never_sends(self):
        self.path.unlink()
        self.assertIn("missing_video", self.invoke()[1])
        self.path.touch()
        self.assertIn("missing_video", self.invoke()[1])
        self.transport.assert_not_called()
    ...
    def test_oversized_file_never_sends(self):
        with patch.object(sender, "MAX_FILE_BYTES", 1):
            self.assertIn("video_too_large", self.invoke()[1])
        self.transport.assert_not_called()
