...
struct PCMStreamingTests {
    ...
    func callbackLedger() throws {
        ...
        let firstResult = ledger.schedule()
        let first = try #require(firstResult)
        let second = ledger.schedule()
        let third = ledger.schedule()
        #expect(second != nil && third != nil)
        #expect(ledger.count == 3)
        let overflow = ledger.schedule()
        #expect(overflow == nil)
        let unknownCompleted = ledger.complete(ticket: UUID(), generation: generation)
        #expect(!unknownCompleted)
        #expect(ledger.count == 3)
        let firstCompleted = ledger.complete(ticket: first, generation: generation)
        let duplicateCompleted = ledger.complete(ticket: first, generation: generation)
        #expect(firstCompleted)
        #expect(!duplicateCompleted)
        #expect(ledger.count == 2)
        let replacement = ledger.schedule()
    ...
    func staleCallbacks() throws {
        ...
        let oldResult = ledger.schedule()
        let oldTicket = try #require(oldResult)
        ledger.reset()
        let newGeneration = ledger.generation
        let newResult = ledger.schedule()
        let newTicket = try #require(newResult)
        #expect(newGeneration != oldGeneration)
        let staleCompleted = ledger.complete(ticket: oldTicket, generation: oldGeneration)
        let wrongGenerationCompleted = ledger.complete(ticket: newTicket, generation: oldGeneration)
        #expect(!staleCompleted)
        #expect(!wrongGenerationCompleted)
        #expect(ledger.count == 1)
        let newCompleted = ledger.complete(ticket: newTicket, generation: newGeneration)
        #expect(newCompleted)
